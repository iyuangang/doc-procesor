...
- 生成数据统计报告
- 命令行界面支持批量处理
- 自适应终端仪表盘，宽屏双栏、窄屏自动堆叠
- 无效记录显示来源、表格行号和失败原因，并另存明细 CSV
- CSV 原子写入，保留所有动态字段
- 损坏文档、部分批处理失败返回非零状态
...
- **问题定位**：真正无效的候选行会在红色面板中标记，并输出 `*_invalid_records.csv`
...
处理过程中若发现真正无效的候选行，主 CSV 仍只包含有效记录，同时在同一输出目录生成 `<文件名>_invalid_records.csv`；目录汇总对应 `combined_results_invalid_records.csv`。明细包含源文档、批次、表格号、Word 表格行号、序号、企业、型号、原始内容和失败原因。多行表头等非车辆行不会计入无效记录。
